...
class MultiHeadAttention(nn.Module):
    # Copia dal notebook (uguale a prima)
    def __init__(self, d_model, n_heads, dropout=0.1):
        ...
        self.dropout = nn.Dropout(dropout)  # Per 2A
...
class GPT2(nn.Module):
    def __init__(
        ...
        self.dropout_emb = nn.Dropout(dropout)  # Per 2A
    ...
    def forward(self, idx, seq_lens):
        ...
        # Intent from CLS (last token)
        cls_tokens = torch.stack([x[i, seq_lens[i]-1] for i in range(B)])
